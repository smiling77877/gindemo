...
type CommentRepository interface {
	...
	// GetCommentyIds 获取单条评论 支持批量获取
	GetCommentByIds(ctx context.Context, ids []int64) ([]domain.Comment, error)
	GetMoreReplies(ctx context.Context, rid, id, limit int64) ([]domain.Comment, error)
}
...
func NewCommentRepo(l logger.LoggerV1, dao dao.CommentDAO) CommentRepository {
	return &CachedCommentRepo{l: l, dao: dao}
}
...
func (c *CachedCommentRepo) GetMoreReplies(ctx context.Context, rid, id, limit int64) ([]domain.Comment, error) {
	cs, err := c.dao.FindRepliesByRid(ctx, rid, id, limit)
	if err != nil {
		return nil, err
	}
