...
import (
	"context"
	"fmt"
	actv1 "gindemo/webook/api/proto/gen/account/v1"
	actmocks "gindemo/webook/api/proto/gen/account/v1/mocks"
	pmtv1 "gindemo/webook/api/proto/gen/payment/v1"
	pmtmocks "gindemo/webook/api/proto/gen/payment/v1/mocks"
	"gindemo/webook/reward/domain"
	"gindemo/webook/reward/integration/startup"
...
func (s *WechatNativeRewardServiceTestSuite) TestPreReward() {
	...
	testCases := []struct {
		...
		mock   func(ctrl *gomock.Controller) (pmtv1.WechatPaymentServiceClient, actv1.AccountServiceClient)
	...
	}{
		{
			name: "直接创建成功",
			mock: func(ctrl *gomock.Controller) (pmtv1.WechatPaymentServiceClient, actv1.AccountServiceClient) {
				pmtclient := pmtmocks.NewMockWechatPaymentServiceClient(ctrl)
				pmtclient.EXPECT().NativePrePay(gomock.Any(), gomock.Any()).
					Return(&pmtv1.NativePrePayResponse{
						CodeUrl: "test_url",
					}, nil)
				actclient := actmocks.NewMockAccountServiceClient(ctrl)
				actclient.EXPECT().Credit(gomock.Any(), gomock.Any()).
					Return(gomock.Any())
				return pmtclient, actclient
			},
			before: func(t *testing.T) {},
			after: func(t *testing.T) {
