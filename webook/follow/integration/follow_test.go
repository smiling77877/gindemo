...
import (
	"context"
	followv1 "gindemo/webook/api/proto/gen/follow/v1"
	"gindemo/webook/follow/integration/startup"
...
func (s *FollowRelationSuite) TestFollowRelation_ADD() {
	testCases := []struct {
		...
		wantVal *followv1.FollowInfoResponse
		wantErr error
	}{
		{
			name: "添加正常",
			...
			wantVal: &followv1.FollowInfoResponse{
				FollowRelation: &followv1.FollowRelation{
					Followee: 1,
					Follower: 2,
				},
			},
...
func (s *FollowRelationSuite) TestFollowRelation_List() {
	...
	for _, tc := range testCases {
		s.T().Run(tc.name, func(t *testing.T) {
			tc.before()
			resp, err := s.server.GetFollowee(context.Background(), &followv1.GetFolloweeRequest{
				Follower: tc.req,
				Limit:    3,
				Offset:   0,
			})
