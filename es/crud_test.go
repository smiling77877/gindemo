...
`
	...
	_, err = s.olivere.Index().Index("user_idx_go").BodyJson(strings.NewReader(data)).Do(ctx)
	require.NoError(s.T(), err)

	_, err = s.olivere.Index().Index("user_idx_go").BodyJson(User{Email: "john@example.com"}).Do(ctx)
	require.NoError(s.T(), err)
}
...
`
	_, err := s.es.Search(s.es.Search.WithContext(ctx), s.es.Search.WithIndex("user_idx_go"),
		s.es.Search.WithBody(strings.NewReader(query)))
	require.NoError(s.T(), err)

	olQuery := olivere.NewMatchQuery("email", "john")
	resp, err := s.olivere.Search().Index("user_idx_go").Query(olQuery).Do(ctx)
	require.NoError(s.T(), err)
