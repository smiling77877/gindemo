...
import (
	...
	smsmocks "gindemo/webbook/internal/service/sms/mocks"
	"gindemo/webbook/pkg/limiter"
	limitermocks "gindemo/webbook/pkg/limiter/mocks"
	...
	testCases := []struct {
		...
		},
		{
			...
			mock: func(ctrl *gomock.Controller) (sms.Service, limiter.Limiter) {
				svc := smsmocks.NewMockService(ctrl)
				l := limitermocks.NewMockLimiter(ctrl)
				l.EXPECT().Limit(gomock.Any(), gomock.Any()).Return(false,
					errors.New("redis限流器错误"))
				return svc, l
			},
