...
func (g *GORMAsyncSmsDAO) GetWaitingSMS(ctx context.Context) (AsyncSMS, error) {
	...
	// 但是我们不是高并发，因为你部署N台机器，才有N个goroutine来查询
	// 并发不过百，随便写
	var s AsyncSMS
	err := g.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		// 为了避开一些偶发性的失败，我们只找1分钟前的异步短信发送
		...
		err = tx.Model(&AsyncSMS{}).Where("id = ?", s.Id).
...
func (g *GORMAsyncSmsDAO) MarkSuccess(ctx context.Context, id int64) error {
	now := time.Now().UnixMilli()
	return g.db.WithContext(ctx).Model(&AsyncSMS{}).Where("id = ?", id).
		Updates(map[string]any{
			"utime":  now,
