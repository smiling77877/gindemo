...
import (
	"context"
	intrv1 "gindemo/webbook/api/proto/gen/intr/v1"
	"gindemo/webbook/interactive/integration/startup"
	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				err = s.db.Where("uid = ? AND biz_id = ? AND biz = ?", 123, 2, "test").First(&likeBiz).Error
				assert.NoError(t, err)
		...
		},
		{
			...
			after: func(t *testing.T) {
				...
				var likeBiz dao.UserLikeBiz
				err = s.db.Where("biz = ? AND biz_id = ? AND uid = ?", "test", 3, 123).First(&likeBiz).Error
				assert.NoError(t, err)
				assert.True(t, likeBiz.Id > 0)
				assert.True(t, likeBiz.Ctime > 0)
				assert.True(t, likeBiz.Utime > 0)
	...
	testCases := []struct {
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				assert.NoError(t, err)
				err = s.rdb.HSet(ctx, "interactive:test:3", "read_cnt", 0, "collect_cnt", 1).Err()
				assert.NoError(t, err)
			},
			wantResp: &intrv1.GetResponse{
				Intr: &intrv1.Interactive{
...
func (s *InteractiveTestSuite) TestGetByIds() {
	...
	for i := 1; i < 5; i++ {
		i := int64(i)
		err := s.db.WithContext(preCtx).Create(&dao.Interactive{
			Id:         i,
			Biz:        "test",
			BizId:      i,
			ReadCnt:    i,
